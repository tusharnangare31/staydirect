...
-- Owners can view payments for their hostels or bookings
...
-- Authenticated students/owners can insert their own payment requests
...
-- Only admins or server triggers can update payment status directly
...
-- Idempotent verification & transition from pending to paid
...
BEGIN
  ...
  -- 2. Verify not already paid (Idempotency)
  ...
  -- 4. If linked to a booking, transition booking to confirmed
  IF v_payment.booking_id IS NOT NULL THEN
    ...
    -- Fetch hostel name and student name for personalized notifications
  ...
  -- 7. If owner subscription payment
...
-- 10. REFUND REQUEST & REVIEW PROCEDURES
-- Student or Owner submits refund request
...
BEGIN
  ...
  -- Only payer or owner or admin can request
  ...
  -- Notify Admins via report/notification table
  INSERT INTO public.notifications (user_id, title, body, type, reference_id)
  VALUES (
    v_payment.user_id,
...
-- Admin Reviews & Approves/Rejects Refund
...
-- 11. ADMIN PAYMENT & REVENUE STATS RPC

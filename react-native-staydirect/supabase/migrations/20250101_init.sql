...
CREATE TABLE IF NOT EXISTS public.hostels (
  ...
  area TEXT NOT NULL, -- e.g. Hinjewadi, Kothrud, Wakad, Viman Nagar, Baner, Kharadi
...
CREATE TABLE IF NOT EXISTS public.rooms (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  hostel_id UUID NOT NULL REFERENCES public.hostels(id) ON DELETE CASCADE,
  sharing_type TEXT NOT NULL, -- 'Single Room', 'Double Sharing', 'Triple Sharing', 'Four Sharing'
...
CREATE TABLE IF NOT EXISTS public.amenities (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  hostel_id UUID NOT NULL REFERENCES public.hostels(id) ON DELETE CASCADE,
  name TEXT NOT NULL, -- 'High-speed Wi-Fi', '3 Times Food / Mess', 'CCTV & Security Guard', 'Power Backup', 'Water Purifier (RO)', 'Laundry / Washing Machine', 'Daily Housekeeping', 'Study Desk & Chair', 'Biometric Access', 'Solar Hot Water'
  icon TEXT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE TABLE IF NOT EXISTS public.bookings (
  ...
  brokerage_fee INTEGER NOT NULL DEFAULT 0, -- Explicitly 0!
...
CREATE TABLE IF NOT EXISTS public.notifications (
  ...
  title TEXT NOT NULL,
  body TEXT NOT NULL,
  type TEXT NOT NULL, -- 'booking_update', 'inquiry', 'message', 'verification'
  reference_id UUID,
  is_read BOOLEAN DEFAULT false,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- 12. OWNER VERIFICATIONS (KYC / Property Proof)
...
-- Hostels: Public read, Owners can insert/update/delete their own
...
-- Hostel Images & Amenities & Rooms: Public read, owner manage
...
-- Favorites: Students manage their own favorites
...
-- Inquiries: Students can view their own, owners can view their received inquiries
...
-- Bookings: Students view their bookings, owners view their bookings
...
CREATE POLICY "Hostel images are public" ON storage.objects
